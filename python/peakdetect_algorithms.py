...
def peakdetect(y_axis, x_axis = None, lookahead = 300, delta=0, delete_false_first_hits=True):
    """
...
if __name__ == "__main__":
    my_data = np.loadtxt("/Users/danramage/tmp/florida_data/tide_stage_data/2004-06-21_04_00.csv", delimiter=',',
                         dtype={'names': ('date', 'water_level'),
                        'formats': ('S21', 'f4')})
    x_vals, y_vals = zip(*my_data)
    _max, _min = peakdetect(y_vals, x_vals, 10, 0, False)
    xm = [p[0] for p in _max]
    ym = [p[1] for p in _max]
    xn = [p[0] for p in _min]
    yn = [p[1] for p in _min]
